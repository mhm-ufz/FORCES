...
module mo_netcdf
  ...
  use mo_netcdf_wrapper, only : &
    ...
    nf90_inq_ncid          => ncw_inq_ncid,          &
    nf90_inq_grp_parent    => ncw_inq_grp_parent,    &
    nf90_inq_grpname       => ncw_inq_grpname,       &
    nf90_def_grp           => ncw_def_grp,           &
    nf90_rename_dim        => ncw_rename_dim,        &
    ...
    NF90_BYTE              => NCW_BYTE,              &
    NF90_SHORT             => NCW_SHORT,             &
    NF90_INT               => NCW_INT,               &
    NF90_INT64             => NCW_INT64,             &
    NF90_FLOAT             => NCW_FLOAT,             &
    NF90_DOUBLE            => NCW_DOUBLE,            &
    NF90_CHAR              => NCW_CHAR,              &
    NF90_FILL_BYTE         => NCW_FILL_BYTE,         &
    NF90_FILL_SHORT        => NCW_FILL_SHORT,        &
    NF90_FILL_INT          => NCW_FILL_INT,          &
    ...
    NF90_HDF5              => NCW_HDF5,              &
    NF90_64BIT_OFFSET      => NCW_64BIT_OFFSET,      &
    NF90_CLASSIC_MODEL     => NCW_CLASSIC_MODEL
...
contains
  ...
  function getVariableIds(self) result(variableIds)
    class(NcGroup), intent(in) :: self
    integer(i4), dimension(:), allocatable :: variableIds
    integer(i4) :: tmp

    allocate(variableIds(self%getNoVariables()))
    call check(nf90_inq_varids(self%id, tmp, variableIds), "Failed to inquire variable ids")
  end function getVariableIds

  function getVariables(self) result(variables)
    class(NcGroup), intent(in) :: self
    ...
    allocate(variables(nvars), varids(nvars))

    varids = self%getVariableIds()
    do i = 1, size(varids)
      variables(i) = NcVariable(varids(i), self)
    end do
  ...
  function getVariableDimensions(self) result(variableDimensions)
    class(NcVariable), intent(in) :: self
    type(NcDimension), allocatable :: variableDimensions(:)
    integer(i4), allocatable :: dimids(:)
    integer(i4) :: ii, ndims

    ndims = self%getNoDimensions()
    allocate(dimids(ndims), variableDimensions(ndims))
    call check(nf90_inquire_variable(self%parent%id, self%id, dimids = dimids), &
            "Could not inquire variable: " // self%getName())

    do ii = 1, ndims
      variableDimensions (ii) = self%parent%getDimension(dimids(ii))
    end do
  end function getVariableDimensions

  function getVariableShape(self) result(variableShape)
    class(NcVariable), intent(in) :: self
    integer(i4), allocatable :: variableShape(:)
    type(NcDimension), allocatable :: dims(:)
    integer(i4) :: ii, ndims

    ndims = self%getNoDimensions()
    allocate(variableShape(ndims), dims(ndims))

    dims = self%getDimensions()
    do ii = 1, size(dims)
      variableShape(ii) = dims(ii)%getLength()
    end do
  end function getVariableShape

  function getVariableShape64(self) result(variableShape64)
    class(NcVariable), intent(in) :: self
    integer(i8), allocatable :: variableShape64(:)
    type(NcDimension), allocatable :: dims(:)
    integer(i4) :: ii, ndims

    ndims = self%getNoDimensions()
    allocate(variableShape64(ndims), dims(ndims))

    dims = self%getDimensions()
    do ii = 1, size(dims)
      variableShape64(ii) = dims(ii)%getLength64()
    end do
  ...
  function getAttributableIds(self) result(attributableIds)
    class(NcAttributable), intent(in) :: self
    integer(i4) :: attributableIds(2)
    select type(self)
    class is (NcGroup)
      attributableIds(1) = self%id
      attributableIds(2) = NF90_GLOBAL
    class is (NcVariable)
      attributableIds(1) = self%parent%id
      attributableIds(2) = self%id
    end select
