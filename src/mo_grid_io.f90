...
contains
  ...
  subroutine out_var_init(self, meta, nc, grid, dims, deflate_level, cache_size, static_contiguous)
    ...
    integer(i4), intent(in), optional :: cache_size !< optional netcdf variable cache size in MB
    logical, intent(in), optional :: static_contiguous !< write static variables as contiguous
    type(NcDimension), allocatable :: var_dims(:)
    logical :: has_layer_var, has_time
    integer(i4), allocatable :: chunksizes(:)
    integer(i4) :: idx, nbytes, y_chunk, z_chunk, t_chunk
    integer(i8), parameter :: max_chunk_bytes = 4294967296_i8
    ...
    use_cache = present(cache_size)
    use_static_contiguous = optval(static_contiguous, .false.)
    if (use_cache) then
      if (cache_size <= 0_i4) call error_message("output_variable: cache_size must be >0 MB: ", self%name)
      cache_bytes = int(cache_size, i8) * 1024_i8 * 1024_i8
      target_chunk_bytes = max(1_i8, cache_bytes / 2_i8) ! aim to fit at least 2 chunks
      if (self%static .and. use_static_contiguous) then
        self%nc = nc%setVariable(self%name, self%dtype, var_dims, contiguous=.true.)
      else
        select case(self%dtype)
          ...
          case("i64", "f64")
            nbytes = 8_i4
          case default
            call error_message("output_variable: unsupported dtype for chunking: ", self%name, ": ", self%dtype)
        end select
        z_chunk = 1_i4
        t_chunk = 1_i4
        denom = int(grid%nx, i8) * int(z_chunk, i8) * int(nbytes, i8)
        if (denom <= 0_i8) call error_message("output_variable: invalid chunk denominator: ", self%name)
        y_chunk = int(target_chunk_bytes / denom, i4)
        y_chunk = max(1_i4, min(grid%ny, y_chunk))
        select case(self%rank)
          case(2_i4)
            allocate(chunksizes(2))
            chunksizes = [grid%nx, y_chunk]
            chunk_bytes = int(grid%nx, i8) * int(y_chunk, i8) * int(nbytes, i8)
          case(3_i4)
            allocate(chunksizes(3))
            if (self%layered) then
              chunksizes = [grid%nx, y_chunk, z_chunk]
              chunk_bytes = int(grid%nx, i8) * int(y_chunk, i8) * int(z_chunk, i8) * int(nbytes, i8)
            else
              chunksizes = [grid%nx, y_chunk, t_chunk]
              chunk_bytes = int(grid%nx, i8) * int(y_chunk, i8) * int(t_chunk, i8) * int(nbytes, i8)
            end if
          case(4_i4)
            allocate(chunksizes(4))
            chunksizes = [grid%nx, y_chunk, z_chunk, t_chunk]
            chunk_bytes = int(grid%nx, i8) * int(y_chunk, i8) * int(z_chunk, i8) * int(t_chunk, i8) * int(nbytes, i8)
          case default
            call error_message("output_variable: unsupported variable rank for chunking: ", self%name)
        end select
        if (chunk_bytes >= max_chunk_bytes) &
          call error_message("output_variable: chunk size exceeds 4GiB limit (reduce cache_size or spatial extent) for variable: ", self%name)
        if (2_i8 * chunk_bytes > cache_bytes) &
          call error_message("output_variable: cache_size too small for two chunks (increase cache_size or use smaller data type/resolution) for variable: ", self%name)
        self%nc = nc%setVariable(self%name, self%dtype, var_dims, chunksizes=chunksizes, cache_size=cache_size, &
                                 cache_preemption=100_i4, deflate_level=deflate_level, shuffle=.true.)
  ...
  subroutine out_var_update_layered_sp(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
    ...
    self%counter = self%counter + 1_i4
  end subroutine out_var_update_layered_sp
  ...
  !> \brief Update output_variable (layered)
  subroutine out_var_update_layered_dp(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
    ...
    self%counter = self%counter + 1_i4
  end subroutine out_var_update_layered_dp

  !> \brief Update output_variable (non-layered)
  subroutine out_var_update_i1(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
  ...
  !> \brief Update output_variable (layered)
  subroutine out_var_update_layered_i1(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
    ...
    self%counter = self%counter + 1_i4
  end subroutine out_var_update_layered_i1

  !> \brief Update output_variable (non-layered)
  subroutine out_var_update_i2(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
  ...
  !> \brief Update output_variable (layered)
  subroutine out_var_update_layered_i2(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
    ...
    self%counter = self%counter + 1_i4
  end subroutine out_var_update_layered_i2
  ...
  !> \brief Update output_variable (layered)
  subroutine out_var_update_layered_i4(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
    ...
    self%counter = self%counter + 1_i4
  end subroutine out_var_update_layered_i4
  ...
  !> \brief Update output_variable (layered)
  subroutine out_var_update_layered_i8(self, data)
    implicit none
    class(output_variable), intent(inout) :: self
  ...
  subroutine in_var_read_layered_sp(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from layered input variable
  subroutine in_var_read_layered_dp(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from input variable
  subroutine in_var_read_i1(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from layered input variable
  subroutine in_var_read_layered_i1(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from input variable
  subroutine in_var_read_i2(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from layered input variable
  subroutine in_var_read_layered_i2(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from layered input variable
  subroutine in_var_read_layered_i4(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  !> \brief read from layered input variable
  subroutine in_var_read_layered_i8(self, flip_y, t_index, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in), optional :: t_index !< current time step
  ...
  subroutine in_var_read_chunk_layered_sp(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read chunk from layered input variable
  subroutine in_var_read_chunk_layered_dp(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read from input variable
  subroutine in_var_read_chunk_i1(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read chunk from layered input variable
  subroutine in_var_read_chunk_layered_i1(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read from input variable
  subroutine in_var_read_chunk_i2(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read chunk from layered input variable
  subroutine in_var_read_chunk_layered_i2(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read chunk from layered input variable
  subroutine in_var_read_chunk_layered_i4(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \brief read chunk from layered input variable
  subroutine in_var_read_chunk_layered_i8(self, flip_y, t_index, t_size, data)
    implicit none
    class(input_variable), intent(inout) :: self
    logical, intent(in) :: flip_y !< flip data along y-axis
    integer(i4), intent(in) :: t_index !< current time step
  ...
  !> \authors Sebastian Müller
  !> \date Apr 2013
  subroutine output_init(self, path, grid, vars, start_time, ref_time, delta, timestamp, deflate_level, grid_double_precision, &
                         layer, layer_vertices, positive_up, cache_size, static_contiguous)
    implicit none
    ...
    integer(i4), intent(in), optional :: cache_size !< optional netcdf variable cache size in MB
    logical, intent(in), optional :: static_contiguous !< write static variables as contiguous (only with cache_size)
    ...
    self%cache_size = 0_i4
    if (present(cache_size)) then
      if (cache_size <= 0_i4) call error_message("output_init: cache_size must be > 0 (MB)")
      self%cache_size = cache_size
    end if
    self%static_contiguous = optval(static_contiguous, .false.)
    ...
    do i = 1_i4, self%nvars
      ...
      call self%vars(i)%init(vars(i), self%nc, self%grid, dims, self%deflate_level, &
                             cache_size=cache_size, static_contiguous=static_contiguous)
    end do
    deallocate(dims)
  ...
  !> \brief Update a variable
  !> \details Add the array given as actual argument to the derived type's component 'data'
  subroutine output_update_i1(self, name, data)
    implicit none
    class(output_dataset), intent(inout) :: self
    character(*), intent(in) :: name !< name of the variable
    integer(i1), intent(in), dimension(:) :: data !< data for current time step
  ...
  !> \brief Update a variable
  !> \details Add the array given as actual argument to the derived type's component 'data'
  subroutine output_update_i2(self, name, data)
    implicit none
    class(output_dataset), intent(inout) :: self
    character(*), intent(in) :: name !< name of the variable
    integer(i2), intent(in), dimension(:) :: data !< data for current time step
  ...
  subroutine input_read_matrix_id_sp(self, var_id, data, current_time)
    ...
    if (size(data, dim=1) /= self%grid%nx .or. &
        size(data, dim=2) /= self%grid%ny) call error_message( &
      "input%read_id: matrix data shape mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: (", num2str(self%grid%nx), ", ", num2str(self%grid%ny), "), got: (", &
  ...
  subroutine input_read_matrix_layered_id_sp(self, var_id, data, current_time)
    ...
    if (size(data, dim=1) /= self%grid%nx .or. &
        size(data, dim=2) /= self%grid%ny .or. &
        size(data, dim=3) /= self%nlayers) call error_message( &
      "input%read_layered_id: layered matrix data shape mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: (", num2str(self%grid%nx), ", ", num2str(self%grid%ny), ", ", num2str(self%nlayers), &
      "), got: (", num2str(size(data, dim=1)), ", ", num2str(size(data, dim=2)), ", ", &
      num2str(size(data, dim=3)), ")")
    t_index = 0_i4
    if (present(current_time) .and. allocated(self%times)) t_index = self%time_index(current_time)
  ...
  subroutine input_read_pack_id_sp(self, var_id, data, current_time)
    ...
    real(sp), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    real(sp), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    ...
    allocate(data_matrix(self%grid%nx, self%grid%ny))
  ...
  subroutine input_read_pack_layered_id_sp(self, var_id, data, current_time)
    ...
    if (size(data, 2) /= self%nlayers) call error_message( &
      "input%read_layered_id: layer count mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%nlayers), ", got: ", num2str(size(data, 2)))
    allocate(data_matrix(self%grid%nx, self%grid%ny, self%nlayers))
  ...
  subroutine input_read_pack_id_dp(self, var_id, data, current_time)
    ...
    real(dp), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    real(dp), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    if (size(data, kind=i8) /= self%grid%ncells) call error_message( &
      "input%read_id: packed data size mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%grid%ncells), ", got: ", num2str(size(data, kind=i8)))
  ...
  !> \brief Read an input variable for a single time step
  subroutine input_read_matrix_i1(self, name, data, current_time)
    implicit none
    class(input_dataset), intent(inout) :: self
    character(*), intent(in) :: name !< name of the variable
    integer(i1), dimension(:, :), intent(out) :: data !< read data
  ...
  subroutine input_read_pack_id_i1(self, var_id, data, current_time)
    ...
    integer(i1), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    integer(i1), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    if (size(data, kind=i8) /= self%grid%ncells) call error_message( &
      "input%read_id: packed data size mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%grid%ncells), ", got: ", num2str(size(data, kind=i8)))
  ...
  !> \brief Read an input variable for a single time step
  subroutine input_read_matrix_i2(self, name, data, current_time)
    implicit none
    class(input_dataset), intent(inout) :: self
    character(*), intent(in) :: name !< name of the variable
    integer(i2), dimension(:, :), intent(out) :: data !< read data
  ...
  subroutine input_read_pack_id_i2(self, var_id, data, current_time)
    ...
    integer(i2), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    integer(i2), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    if (size(data, kind=i8) /= self%grid%ncells) call error_message( &
      "input%read_id: packed data size mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%grid%ncells), ", got: ", num2str(size(data, kind=i8)))
  ...
  subroutine input_read_pack_id_i4(self, var_id, data, current_time)
    ...
    integer(i4), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    integer(i4), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    if (size(data, kind=i8) /= self%grid%ncells) call error_message( &
      "input%read_id: packed data size mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%grid%ncells), ", got: ", num2str(size(data, kind=i8)))
  ...
  subroutine input_read_pack_id_i8(self, var_id, data, current_time)
    ...
    integer(i8), dimension(:), intent(out) :: data !< read data
    type(datetime), intent(in), optional :: current_time !< current time step
    integer(i8), allocatable :: data_matrix(:, :)
    if (var_id < 1_i4 .or. var_id > self%nvars) call error_message("input%read_id: invalid variable id")
    if (size(data, kind=i8) /= self%grid%ncells) call error_message( &
      "input%read_id: packed data size mismatch for variable ", self%vars(var_id)%name, &
      ". Expected: ", num2str(self%grid%ncells), ", got: ", num2str(size(data, kind=i8)))
  ...
  integer(i4) function input_time_index(self, current_time)
    ...
    if (t_val == self%t_bounds(left)) then
      input_time_index = 0_i4 ! treat start time as special case
      return
    else if ((t_val < self%t_bounds(left)) .or. (t_val > self%t_bounds(right+1_i4))) then
      call error_message("input%time_index: read time (", current_time%str(), ") not covered by file: ", self%path)
    end if
