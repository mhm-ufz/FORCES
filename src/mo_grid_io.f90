...
contains
  ...
  subroutine out_var_write(self, t_index)
    ...
    select case(self%kind)
      case("sp")
        if (self%layered) then
          if (self%avg) self%data_layered_sp = self%data_layered_sp / real(self%counter, sp)
          ...
          call self%nc%setData(buffer_sp, start=start)
          deallocate(buffer_sp)
          self%data_layered_sp = 0.0_sp
        else
          if (self%avg) self%data_sp = self%data_sp / real(self%counter, sp)
          call self%nc%setData(unpack(self%data_sp, self%grid%mask, nodata_sp), start=start)
          self%data_sp = 0.0_sp
        end if
      case("dp")
        if (self%layered) then
          if (self%avg) self%data_layered_dp = self%data_layered_dp / real(self%counter, dp)
          ...
          call self%nc%setData(buffer_dp, start=start)
          deallocate(buffer_dp)
          self%data_layered_dp = 0.0_dp
        else
          if (self%avg) self%data_dp = self%data_dp / real(self%counter, dp)
          call self%nc%setData(unpack(self%data_dp, self%grid%mask, nodata_dp), start=start)
          self%data_dp = 0.0_dp
        end if
      case("i1")
        if (self%layered) then
          if (self%avg) self%data_layered_i1 = int(int(self%data_layered_i1, kind=i4) / self%counter, kind=i1)
          ...
          call self%nc%setData(buffer_i1, start=start)
          deallocate(buffer_i1)
          self%data_layered_i1 = 0_i1
        else
          if (self%avg) self%data_i1 = int(int(self%data_i1, kind=i4) / self%counter, kind=i1)
          call self%nc%setData(unpack(self%data_i1, self%grid%mask, nodata_i1), start=start)
          self%data_i1 = 0_i1
        end if
      case("i2")
        if (self%layered) then
          if (self%avg) self%data_layered_i2 = int(int(self%data_layered_i2, kind=i4) / self%counter, kind=i2)
          ...
          call self%nc%setData(buffer_i2, start=start)
          deallocate(buffer_i2)
          self%data_layered_i2 = 0_i2
        else
          if (self%avg) self%data_i2 = int(int(self%data_i2, kind=i4) / self%counter, kind=i2)
          call self%nc%setData(unpack(self%data_i2, self%grid%mask, nodata_i2), start=start)
          self%data_i2 = 0_i2
        end if
      case("i4")
        if (self%layered) then
          if (self%avg) self%data_layered_i4 = self%data_layered_i4 / self%counter
          ...
          call self%nc%setData(buffer_i4, start=start)
          deallocate(buffer_i4)
          self%data_layered_i4 = 0_i4
        else
          if (self%avg) self%data_i4 = self%data_i4 / self%counter
          call self%nc%setData(unpack(self%data_i4, self%grid%mask, nodata_i4), start=start)
          self%data_i4 = 0_i4
        end if
      case("i8")
        if (self%layered) then
          if (self%avg) self%data_layered_i8 = self%data_layered_i8 / int(self%counter, i8)
          ...
          call self%nc%setData(buffer_i8, start=start)
          deallocate(buffer_i8)
          self%data_layered_i8 = 0_i8
        else
          if (self%avg) self%data_i8 = self%data_i8 / int(self%counter, i8)
          call self%nc%setData(unpack(self%data_i8, self%grid%mask, nodata_i8), start=start)
          self%data_i8 = 0_i8
        end if
    end select
  ...
  subroutine output_define_z_axis(self, double_precision)
    ...
    use_double = double_precision
    dtype = "f32"
    if (use_double) dtype = "f64"
