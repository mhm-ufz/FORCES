...
contains
  ...
  subroutine points_series_output_init(self, path, points, vars, time_axis, point_dim_name, &
    ...
    if (has_time_bounds) then
      call self%time_axis%init_cf(time_values, time_units, bounds=time_bnds, timestamp=time_axis%timestamp)
    else
      call self%time_axis%init_cf(time_values, time_units, timestamp=instant_timestamp)
    end if
    self%time_axis%timestep = time_axis%timestep
    self%deflate_level = optval(deflate_level, 6_i4)
    dimname = optval(point_dim_name, "point")
